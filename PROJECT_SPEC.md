...
Digital MIPS-datapath blocks in the **Logic** category, drawn with native CircuiTikZ shapes — `flipflop` (the `flipflop D/SR/JK/T` presets) and the configurable `muxdemux` (also exposed by the predefined `ALU`, `demux`, and `one bit adder` styles). Each is a **centre-placed** multi-terminal node (`anchor_pin` null, like the op amp). The native anchors are off the 0.25-GU grid (e.g. ±0.56, ±0.84, ±0.98), so generation applies a **best-effort grid alignment** (`renderer.best_alignment`): a per-axis scale (independent `sx`/`sy` within an anisotropy cap, §4) that lands as many pins as possible on the grid (ties broken toward 1.0), baked into the rendered geometry and stored as the symbol's `scale`. Flip-flops align **fully** (the scale lands all four corners on the grid); a mux/demux aligns its data lines while the slanted **select** pins, and the **ALU** operands, can't be brought on-grid by the scale and stay off-grid (a wire snaps onto them via the magnet, exactly as for a scaled logic gate — §5.6, §6.4; only those kinds are exempt from the on-grid-pin invariant). No lead bridges: pins sit at the scaled anchors. They carry **no `l` label slot** — the raw pgf shapes reject the bipole `l=` quick key and are self-labelled by their pin glyphs (D/Q/CLK, A/B, …).
...
- **Two-terminal bipoles** (`emission: "path"`): more resistors/sensors (`varistor`, `photoresistor`, NTC/PTC `thermistor`), capacitors (`vC`, `feC`, `cC`, `sC`, `piezoelectric`), inductors (`vL`, `sL`), thyristors (`thyristor`/SCR, `triac`), sources (`dcvsource`, `dcisource`, `vsourcesquare`, `vsourcetri`, `vsourceN`), switches (`spst`, `cute open/closed switch`, `reed`, `toggle switch`), instruments (`oscope`, `rmeter`), transducers (`loudspeaker`, `mic`, `buzzer`), signal-processing/RF **Blocks** (`amp`, `adc`, `dac`, `lowpass`/`highpass`/`bandpass`/`allpass`, `phaseshifter`, `detector`, `vco`), and misc (`afuse`, `squid`, `bulb`).
  ...
  - **Potentiometer wiper (an off-axis terminal on a path device).** A potentiometer is a *three*-terminal device; `pR` (american) and `epot` (european potentiometer) carry a third **`wiper`** pin in addition to the two axial terminals. Exactly like the thyristor gate, the wiper is the CircuiTikZ `wiper` anchor (offset `(1.0, −0.56)` GU — mid-body, off-axis, off the 0.25 grid → magnet-connected), kept as `pins[2]`, with `anchor: null` so a wire connects to it **by coordinate** (no codegen change; coincidence verified at all eight rotation×mirror cases by `test_codegen.test_potentiometer_wiper_pin_coincides_with_circuitikz_anchor`). `pR`/`epot` join the off-grid-pin invariant exemption. (The variable resistor `vR` and resistive sensor `sR` also expose `wiper`/`tip` anchors in CircuiTikZ, but those are label/positioning points on two-terminal devices, not electrical terminals, so they are not modelled as pins.)
...
- **Centre-placed multi-terminal nodes** (best-effort grid alignment, like the digital blocks): electron **Tubes** (`triode`, `diodetube`, `pentode`, `tetrode` — the multi-grid tubes expose the extra grid taps: `tetrode` adds a `screen` grid, `pentode` adds `screen` + `suppressor`), the fully-differential op-amp (`fd op amp`), the Schmitt triggers (`schmitt`/`invschmitt`), the `gyrator`, and the **SPDT/rotary switches** (`cute spdt up/down/mid`, `rotaryswitch`). These keep native off-grid anchors (magnet-connected) and join `mux`/`demux`/`ALU`/`adder` in the on-grid-pin invariant exemption (`tests/test_registry._OFFGRID_PIN_KINDS`). A centre-placed node scales **per-axis only within an anisotropy cap** (`best_alignment`/`_scale_for`, §4); beyond that ratio it falls back to a single **uniform** scale, because a strongly non-uniform scale shears the drawn strokes (e.g. a slanted switch blade) in the LaTeX output while the canvas re-strokes every path at a bucketed uniform width — desyncing canvas from output. The SPDT/rotary switches were originally authored anchor-pinned, whose forced-symmetric grid targets produced exactly that shear; they are centre-placed so the cap can hold their scale near-uniform (`tests/test_registry.test_node_scale_within_anisotropy_cap`).
...
**Centre taps (sub-node anchors).** Each transformer additionally exposes the two winding **centre taps** — `tap_p` (primary) and `tap_s` (secondary) — as `pins[4]`/`pins[5]`. These are *not* anchors of the transformer node itself: CircuiTikZ draws each winding as an internal coil sub-node (`<node>-L1` primary, `<node>-L2` secondary), and the tap is that coil's `midtap` anchor. So a tap's registry `anchor` is a **sub-node reference** prefixed with `-` (`-L1.midtap`/`-L2.midtap`); codegen detects the leading `-` and emits `(node-L1.midtap)` rather than `(node.…)` (the only special case in `pin_coord_to_ref`, `app/codegen/circuitikz.py`). Connecting by the *named* anchor (not by coordinate, unlike the wiper/gate) is what keeps the tap correct under the transformer's `transform shape` rotation/mirroring — the symbolic anchor is reoriented exactly as the four winding terminals are (`test_codegen.test_transformer_centre_taps_emit_subnode_anchor_refs`). The tap offsets are off-grid (american ≈`(±0.4, 0)`, cute/european ≈`(±0.2, 0)` after the baked scale; the position shifts with the coil count, which Heaviside does not expose, so the default is stable) so a wire snaps onto them via the magnet; the six transformer kinds join the off-grid-pin invariant exemption.
...
A **line-hop** marks one wire jumping over another where they cross *without connecting*, so the crossing reads unambiguously as "no connection". In the LaTeX output it is a CircuiTikZ **`jump crossing` node** (the package's native crossing facility — see §7.6); on the canvas it is an approximation of that node's hop arc. Like junction dots and open circles it is **pure decoration, derived and never stored** — computed by `wire_crossings(schematic, default_on)` (returns a list of `WireHop(point, wire_id, orientation, seg_index, crossed_wire_id)`, where `crossed_wire_id` is the *other* (hopped-over) wire; `default_on` is the global preference, see "Toggle" below).
...
- **Which wire hops.** At a crossing, a wire may hop only if its `hop_mode` allows it there: `"never"` can never hop (but may be hopped *over*), `"always"` can always hop, `""` (default) can hop only when the global preference is on. Among the wires allowed to hop, the winner is chosen by **tier** (`"always"` outranks default), then higher `z_order`, then later position in `schematic.wires`. If neither may hop, no crossing is drawn. So `"always"` forces a hop even with the global preference off or a lower `z_order`, and `"never"` cleanly hands the hop to the crossing wire. The hopping wire carries the arc and the crossed wire the gap — on the canvas the arc bulges upward for a horizontal hopper and **left** for a vertical one, matching the CircuiTikZ `jump crossing` node (which is rotated 90° for a vertical hopper, §7.6).
...
- **Toggle.** Drawing hops is a **display preference** (`display/line_hops`, §10.8), **on by default** (the schematic-drawing convention). It is passed as the `default_on` argument of `wire_crossings()` — governing only default-mode (`hop_mode==""`) wires, so per-wire `"always"`/`"never"` overrides still apply when it is off. The scene flag `_line_hops` (set via `SchematicScene.set_line_hops`) and `generate(..., mark_line_hops=…)` therefore **always** run the detector and feed each wire item / the LaTeX output its hops (§7.6) — the preference no longer gates the call, only the default-mode wires. In export, each crossing becomes a `jump crossing` node and both wires break to its anchors (§7.6). On the canvas, the hopping wire paints a semicircular arc approximating that node, sized to match it: the half-width to the node anchors is `HOP_HALF_GU` (0.14 GU — **measured** from CircuiTikZ's default `bipoles/crossing/size`, so the on-screen hop lines up with the export) and the arc radius is `HOP_ARC_RADIUS_GU` (0.4·half). The crossed wire's sub-pixel gap is omitted on canvas (the node draws it in export). Each hop is matched to the segment it lies on **by coordinate**, so a hop that no longer falls on the polyline is simply skipped — the same hop list works for committed and live-preview geometry.
...
- **Double-clicking a node-style component** edits its **node text** in place (`ComponentItem._node_text_editor`, a sibling `LabelTextItem` centred where the node text renders, pre-filled with the verbatim `node_text`, committing via `edit_component_node_text` / `EditNodeTextCommand`; Enter/focus-loss commits, Escape cancels). A node-style component's **`node[…]` options are not shown on the canvas and are not editable there** — they are raw CircuiTikZ edited only through the inspector's *Node options* field (showing them beside the node text reads as clutter). So double-clicking either the node body or the node-text label opens the node-text editor; for a **non-node-style** component the body/label still opens the **options** editor (the scene routes by whether `is_node_style(kind)`).
...
- **Double-clicking the component body** in **Select** mode opens an in-place editor **and** emits `SchematicScene.component_double_clicked(component_id)` to open the **Properties Panel** simultaneously. It opens the **options** editor — except for a **node-style component**, where it opens the **node-text** editor (a node's `node[…]` options are inspector-only, never edited on the canvas; §5.4). While editing, all key events (Backspace, Delete, arrows, Escape) are routed to the editor rather than the canvas hotkeys.
...
- **Paste** has two entries that share one commit primitive, `paste(at=…)`:
  - **Interactive (`Ctrl+V` / Edit-menu, `begin_paste`)** — the clipboard group becomes **cursor-following ghosts** (a sub-state of PLACE mode): one `set_ghost` `ComponentItem` per copied component and a static `WirePreviewItem` polyline per copied wire, translated rigidly so the group's **bounding-box top-left** (the clipboard min corner, `_paste_anchor_gu`) sits at the snapped cursor. A **left-click** commits at the cursor; **Escape** or a **right-click** cancels with nothing pasted. This lets the user position pasted pins instead of pasting blind, so a paste no longer silently splits a wire or connects to whatever sat under a fixed offset. Ghost state (`_paste_anchor_gu`, `_paste_ghosts`) is torn down by `_cancel_placement`, so any mode change (Escape, tool switch) cancels cleanly.
  - **Context-menu "paste here" (§6.9)** — passes the right-click point straight to `paste(at=…)`, committing immediately at that deliberate point (no ghost step).
- The commit primitive `paste(at)` re-instantiates the clipboard with **new UUIDs**, pushes one `MacroCommand("Paste")`, returns to SELECT mode, and selects the pasted items; pins landing on an existing wire split it (bundled into the same command). The group is anchored so its bounding-box top-left lands at *at* (a grid-snapped point), so pasted geometry stays grid-valid. Called with no *at* it falls back to a fixed **(1, 1) GU** offset (programmatic/legacy path; no UI uses it). No-op when the clipboard is empty.

All three are also on the menu-bar **Edit** menu (Cut/Copy/Paste, standard shortcuts) and the right-click context menu (§6.9). The Edit-menu Paste `QAction` is connected through an argument-swallowing lambda (`lambda: scene.begin_paste()`), **never** directly to a slot that takes a positional argument: `QAction.triggered` emits a `checked` boolean that — when the slot was `scene.paste` — bound to `paste`'s `at` parameter, took the "paste here" branch, and subscripted a bool (regression [#33](https://github.com/whileman133/Heaviside/issues/33)). Cut/Copy take no arguments, so they may connect directly.
...
**Line-hops** (optional) — when `generate()` is called with `mark_line_hops=True`, each crossing returned by `wire_crossings()` (§6.4) is emitted as a CircuiTikZ **`jump crossing` node** (the package's native crossing, used as the maintainers intend): `\node[jump crossing] (xingK) at (x,y) {};`, rotated `90°` when the hopper is **vertical** so the arc lands on the vertical arm. The nodes are emitted **up front**, before any `\draw` (TikZ has no forward node references). **Both** wires at the crossing then break to the node's anchors — the hopper to `.west`/`.east` (the arm carrying the arc), the crossed wire to `.north`/`.south` (the gapped arm) — so a wire's path reads `… -- (xingK.west) (xingK.east) -- …` (the space starts a fresh sub-path), leaving the crossing itself for the node to draw. The anchor for each arm is chosen by its position relative to the crossing (`_crossing_anchor`), validated against the shape; multiple crossings on one wire chain (`(xing0.east) -- (xing1.west)`). Node positions pass through the same `_y` Y-flip as the rest of the figure. The default `bipoles/crossing/size` is used (no `\ctikzset` override); its measured anchor offset (0.14 GU) is `HOP_HALF_GU`, which the canvas matches. The flag is driven by the **Draw line-hops** display preference (§10.8), defaults to `False` at the `generate()` layer (the app passes the preference, which defaults on), so output is unchanged unless requested. *Limitation:* CircuiTikZ crossing nodes are fixed-size, so two crossings closer than ~0.28 GU (rare — adjacent grid cells) can visually overlap; there is no auto-shrink (unlike the former hand-drawn bump).
...
- **Reflects what is compiled and exported.** The panel calls `generate()` with
  the same display flags as the preview/export paths — `mark_unconnected_pins`
  *and* `mark_line_hops` (§7.6) — so a line-hop bump or an unconnected-pin circle
  shown in the figure also appears in the source. The **only** intended
  difference is the Y convention: the panel uses the canvas Y-down for
  readability while the compile path emits Y-up (`y_flip=True`); the emitted
  primitives are otherwise identical. A preference change refreshes the panel
  (`refresh()`), so toggling line-hops updates the displayed source immediately.
...
└── tests/
    ...
    ├── test_sourcepanel.py        # source panel reflects compiled output (line-hops/pin display flags)
...
fallback), **line-hops as `jump crossing` nodes** (node emission + rotation for a
vertical hopper, both wires breaking to the right anchors, Y-flip of the node
position; the real-pdflatex compile is in `test_preview_render.py`),
markers/labels and their Y-flip-aware anchoring, drawing annotations,
...
pinned: the thyristor/triac gate and the potentiometer (`pR`/`epot`) wiper coincide with their
CircuiTikZ anchors at all eight rotation×mirror cases (`test_thyristor_gate_pin_coincides_with_circuitikz_anchor`,
`test_potentiometer_wiper_pin_coincides_with_circuitikz_anchor` — connected by coordinate), and a
transformer's primary/secondary centre taps emit the internal-coil **sub-node** anchor references
`(node-L1.midtap)`/`(node-L2.midtap)` and compile under rotation/mirror across all coil styles
