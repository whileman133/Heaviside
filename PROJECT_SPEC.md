...
All three are also on the menu-bar **Edit** menu (Cut/Copy/Paste, standard shortcuts) and the right-click context menu (§6.9). The Edit-menu Paste `QAction` is connected through an argument-swallowing lambda (`lambda: scene.paste()`), **never** directly to `scene.paste`: `QAction.triggered` emits a `checked` boolean that would otherwise bind to `paste`'s `at` parameter, take the "paste here" branch, and subscript a bool (regression [#33](https://github.com/whileman133/Heaviside/issues/33)). Cut/Copy take no arguments, so they may connect directly.
