...
**Length-resizable path symbols.** Every path (`to[…]`) symbol with two axial terminals (R, capacitor, diode, source, **and** the extra-pin devices: thyristor/triac, potentiometer, inductor) is **length-resizable** (`library.is_length_resizable`, `ComponentDef.resizable`): the user drags either end handle — or sets the inspector **Length** field (§10.3) — to lengthen the device. The **body keeps its natural size and the leads extend**, exactly as CircuiTikZ renders `to[…]` between two coordinates that are farther apart. Implementation: the length is stored in `span_override` as `(length, 0)` in the component's local frame; `component_pin_positions` places the far terminal at `position + rotate((length, 0))` (off-grid when rotated), the near terminal stays, and any **body-relative pin** (index ≥ 2 — a gate, wiper, or midtap) **shifts by half the length increase** so it stays attached to the body centred on the new midpoint. Codegen's `_two_terminal_line` emits the stretched axial endpoints (zero codegen change); the extra pin's wire connects at its shifted coordinate / named anchor, which CircuiTikZ draws at the same body-relative spot. The canvas reuses the resizable-annotation item infrastructure via `_ResizablePathItem` (a `_ResizableTwoTerminalItem` that paints the real SVG **symbol** instead of a line): `ComponentItem.paint`/`boundingRect` become span-aware (`_span_extension` — translate the whole symbol to the new midpoint, draw straight stub leads to each terminal, and draw the body-relative pin dots shifted to match). **Length only, along the component's existing axis** (orientation still comes from the rotation, §6.x), and **lengthen-only** (floored at the natural length — shrinking below it would overflow the body past the pins). The dragged end uses a **grid magnet** (`DragController._axis_pin` → `_snap_axis_length`): it snaps to the nearest 0.25-GU grid node **that lies on the axis** within `_GRID_MAGNET_GU`, else lands free off-grid ("snaps to grid but not required"). So an axis-aligned device snaps every 0.25, while a 45° device snaps to the diagonal grid crossings it actually passes through (spacing 0.25·√2) — not to 0.25 length steps. The raw cursor is plumbed through `preview_endpoint_drag`/`commit_endpoint_drag`; connected wires follow the moved pin (the existing `ResizeCommand` reshape). **Excluded**: nodes, grounds, and the bespoke annotations/boxes (which have their own resize). Helps draw, e.g., the 45°-diode bridge rectifier of [#38](https://github.com/whileman133/Heaviside/issues/38) (rotate to 45°, then lengthen to close the diamond).
...
- **Pin-to-pin alignment (off-grid, §5.9).** While a **single** component is dragged, its origin also snaps so one of its pins lands on another component's **off-grid pin axis line** — the same "artificial grid line" mechanism wire ends use (`_all_offgrid_pin_axes`). So a resistor can be lined up exactly with a transistor's off-grid collector/base, eliminating the tiny jog a 0.25-grid-only snap would leave. `scene._resolve_component_origin` builds the candidate origins (for each pin offset and each other component's off-grid axis, `axis − offset`) and picks the nearer of those or the plain grid node (ties → the grid, so it is not sticky); a faint **guide line** (`_show_guides`) marks the axis the pin landed on. The component never aligns to its own pins (the dragged id is excluded). The resolver runs in both the live preview (`mouseMoveEvent`) and the commit (`commit_component_drag`) so they agree — the resulting origin may be **off-grid**, which is valid (only wire vertices are grid-constrained, §4.5 invariant 3). Group drags keep the plain grid snap.
...
The pure (Qt-free) command layer is unit-tested directly (undo/redo behaviour also exercised via §13.3). Coverage spans every command class — `MoveCommand` (wire-following: endpoint-follow, rigid translate, auto-elbow, re-stretch lead off a multi-wire junction, whole-wire move with junction taps), `Split`/`MergeWireCommand`, `MoveWireVertexCommand` (reshape/simplify, collapse-removes-wire), `DeleteCommand`, `MacroCommand` (composition and failure-unwinding), `MoveOptionsLabelCommand`, the `ResizeCommand`/`MoveEndpointCommand` span/position math, the `Set*` attribute setters, and `GroupRotateCommand` (centroid rotation, boundary-wire reshape, mirrored −step; 90° and 45° steps). The **45° rotation** (components orient in 45° increments, §6.x) is pinned in `test_model.py` (`test_component_45_degree_rotation_is_valid`, `test_rotate_vector_right_angles_are_exact`/`…_45_uses_trig`, `test_component_pin_positions_at_45`), `test_codegen.test_bipole_at_45_emits_diagonal_endpoints`, `test_scene.py` (`test_rotate_action_steps_45_for_component_only_selection`, `test_rotate_action_steps_90_when_a_wire_is_selected`), and the inspector control `test_properties.test_transform_section_offers_45_degree_rotations` (all eight 45° buttons; a diagonal button rotates undoably). **Length-resizable path symbols** (§5.7) are pinned by `test_components_library.test_path_symbols_are_length_resizable` (R/capacitor/diode/source **and** the extra-pin L/thyristor/pot resize; nodes, grounds, and bespoke annotations do not), `test_model.test_resized_path_symbol_shifts_body_relative_pins` (the inductor midtap / thyristor gate shift by half the length increase), `test_codegen.test_resized_path_symbol_emits_stretched_coords` (the stretched `to[…]` endpoints), `test_scene.py` (`test_set_component_length_lengthens_path_symbol` — inspector path, far-pin move, valid, undo, natural-length floor; `test_path_symbol_endpoint_drag_grid_magnet_and_offgrid` — magnet snaps near a node, lands free off-grid otherwise; `test_path_symbol_lengthens_along_its_45_degree_axis` — axis-projected), and `test_properties.test_path_length_section_resizes_undoably` (the Length field shows for R and the thyristor, hidden for op amp, sets length undoably). The non-trivial invariants are the focus: exact undo, **decoration preservation** across split/merge/collapse with **verbatim restore at the original list index**, block-diagram rect-edge / circle-cardinal connections following move and anchored-scale resize, execution-time split-index resolution (clean no-op when bundled reshapes move the point), `point_key` float-noise tolerance (§4.5), and the `UndoStack` save-point / `record()` semantics.
