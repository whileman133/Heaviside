...
The file is generated by ``components/generate_library.py`` and holds, for every
...
def gate_size_keys(kind: str) -> dict | None:
    """For a manual-library logic gate sized via the CircuiTikZ body ``height``/
    ``width`` ``\\ctikzset`` keys, the ``{"path": "tripoles/<kw>", "height": <default>,
    "width": <default>}`` baked by the generator; ``None`` otherwise (not/buffer gates,
    which the keys don't size — they fall back to the node ``xscale``/``yscale``).

    The american and/or/nand/nor/xor/xnor families set their body size with these keys
    (the CircuiTikZ-recommended way), which redraws the symbol at native stroke width
    rather than stretching it like ``xscale``/``yscale``. The per-instance size is the
    default × the anisotropic resize factors (:func:`app.schematic.model.node_resize_factors`):
    ``height = height_default × hf``, ``width = width_default × wf``."""
    return load_library().get(kind, {}).get("size_keys")
...
def to_component_def(kind: str, entry: dict) -> ComponentDef:
    ...
    return ComponentDef(
        ...
        # Measured node-text anchor offset (components/generate_library.py); (0,0)
        # when absent.
...
def _is_diode_entry(e: dict) -> bool:
    """True for a two-terminal member of the ``Diodes`` family — the symbols
    CircuiTikZ's ``diodes/scale`` key resizes.  Keyed off ``category``/``emission``/
    pin-count so it works for both the curated library (whose diodes carry a ``filled``
    variant) and the manual-scraped one (whose fill lives in distinct ``empty``/``full``/
    ``stroke`` *kinds*, with no variant).  Tripoles in the family (thyristor/triac/GTO/
    PUT) are excluded: their gate anchor moves with the scale and would desync the canvas
    pin from the export.  Must match :func:`app.components.generate.is_diode`, which bakes
    these bodies at :data:`DIODE_SYMBOL_SCALE`."""
    return (e.get("category") == "Diodes"
            and e.get("emission") == "path"
            and len(e.get("pins", [])) == 2)


def is_diode(kind: str) -> bool:
    """True for a diode-family symbol (see :func:`_is_diode_entry` — the same test
    :func:`build_codegen_tables` uses for ``diode_kinds``). These are drawn with the
    document's ``diode_scale`` body size (the ``\\ctikzset{diodes/scale=…}`` key) on
    both the canvas and the export."""
    return _is_diode_entry(load_library().get(kind, {}))
